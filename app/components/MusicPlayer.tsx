...
export default function MusicPlayer(){
 ...
 useEffect(()=>{if(!expanded)return;const close=(e:KeyboardEvent)=>{if(e.key==='Escape'){setExpanded(false);launcher.current?.focus()}};window.addEventListener('keydown',close);return()=>window.removeEventListener('keydown',close)},[expanded]);
 ...
 return <><audio ref={audio} src={available?track?.url:undefined} preload="none" loop={settings?.mode==='daily'||tracks.length===1} onPlay={()=>setPlaying(true)} onPause={()=>setPlaying(false)} onEnded={()=>{if(settings?.mode==='playlist')setIndex(i=>(i+1)%tracks.length)}} onError={()=>setMessage('Lagu tidak dapat dimuat. Coba lagu berikutnya.')} />{available&&<div className="fv-music-dock">{expanded&&<aside id="fv-music-controls" className="fv-music-player" aria-label="Pemutar musik"><div><small>{settings.mode==='daily'?'Music of the Day':'Playlist Family Vault'}</small><strong>{track.title}</strong><small>{track.artist}</small></div><button type="button" onClick={toggle}>{playing?'Pause':'Putar Musik'}</button>{settings.mode==='playlist'&&tracks.length>1&&<button type="button" aria-label="Lagu berikutnya" onClick={()=>setIndex(i=>(i+1)%tracks.length)}>▶▶</button>}<label>Volume<input aria-label="Volume musik" type="range" min="0" max="1" step=".05" value={volume} onChange={e=>{const v=Number(e.target.value);setVolume(v);localStorage.setItem('fv-music-volume:'+uid.current,String(v))}}/></label><button type="button" onClick={()=>{wanted.current=false;localStorage.setItem('fv-music-off:'+uid.current,'1');audio.current?.pause();setMessage('Musik dimatikan. Tekan Putar Musik untuk mengaktifkan.')}}>Matikan</button>{message&&<small role="status">{message}</small>}</aside>}<button ref={launcher} type="button" className={'fv-music-launcher'+(playing?' is-playing':'')} aria-label={expanded?'Tutup kontrol musik':playing?'Buka musik yang sedang diputar':'Buka kontrol musik'} aria-expanded={expanded} aria-controls={expanded?'fv-music-controls':undefined} title={expanded?'Tutup musik':'Musik'} onClick={()=>setExpanded(v=>!v)}>{expanded?'×':'♫'}{playing&&!expanded&&<i aria-hidden="true"/>}</button></div>}</>
}
