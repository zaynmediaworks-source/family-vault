...
type Action = {kind: ActionKind; id: string; title: string} | null;
...
export default function WealthExperienceV3({houses}:{houses:R[]}){
  ...
  async function remove(table:string,id:string,label:string){
    ...
    if(r.error)setToast(r.error.message); else await refresh(`${label} dihapus.`);
  }
  ...
  async function deleteDebt(d:R){
    ...
    if(r.error)setToast(r.error.message); else await refresh('Hutang dihapus.');
  }
  ...
  async function deleteWish(w:R){
    ...
    if(r.error)setToast(r.error.message); else await refresh('Wishlist dihapus.');
  }

  async function runRecurring(){
    const due=recurring.filter(x=>x.active&&x.next_run<=today());
    if(!due.length){setToast('Belum ada transaksi otomatis yang jatuh tempo.');return;}
    let count=0;
    for(const x of due){
      let r:any;
      if(x.kind==='income')r=await supabase.from('income').insert({household_id:hid,source:x.name,amount:x.amount,happened_at:x.next_run,note:'Recurring',created_by:uid});
      else if(x.kind==='expense')r=await supabase.from('expenses').insert({household_id:hid,name:x.name,category:x.category||'Recurring',amount:x.amount,happened_at:x.next_run,note:'Recurring',created_by:uid});
      else if(x.kind==='obligation')r=await supabase.from('obligations').insert({household_id:hid,name:x.name,amount:x.amount,status:'belum',due_date:x.next_run,note:'Recurring',created_by:uid});
      else if(x.kind==='saving'&&x.target_id)r=await supabase.from('saving_transactions').insert({saving_id:x.target_id,amount:x.amount,happened_at:x.next_run,created_by:uid});
      if(r?.error)continue;
      const d=new Date(x.next_run+'T00:00:00');
      if(x.frequency==='weekly')d.setDate(d.getDate()+7);
      else if(x.frequency==='yearly')d.setFullYear(d.getFullYear()+1);
      else d.setMonth(d.getMonth()+1);
      await supabase.from('recurring_transactions').update({next_run:d.toISOString().slice(0,10)}).eq('id',x.id);
      count++;
    }
    await refresh(`${count} transaksi otomatis diproses.`);
  }

  const house=houses.find(x=>x.id===hid);

  return <>
    {section&&<div className="wealth-overlay"><div className="wealth-panel">
      <header className="investment-overlay-head">
        <div><div className="kicker">Family Vault</div><h1>{section}</h1><p className="muted">{house?.name}</p></div>
        <div className="action-row">
          <select value={hid} onChange={e=>{setHid(e.target.value);load(e.target.value)}}>{houses.map(h=><option key={h.id} value={h.id}>{h.name}</option>)}</select>
      ...
      {section==='Otomatis'&&<Recurring rows={recurring} setAction={setAction} onRun={runRecurring} remove={remove} refresh={refresh}/>} 
    </div></div>}
    {action&&<ActionModal action={action} hid={hid} uid={uid} savings={savings} wishes={wishes} parts={parts} debts={debts} onClose={()=>setAction(null)} onSaved={async m=>{setAction(null);await refresh(m)}}/>}
...
function Savings({savings,tx,setAction,remove}:{savings:R[];tx:R[];setAction:(a:Action)=>void;remove:(t:string,id:string,l:string)=>void}){
  return <section>
    <div className="panel-line"><div><div className="kicker">Dana & Target</div><h2>Tabungan / Dana Darurat</h2></div><button className="btn" onClick={()=>setAction({kind:'newSaving',id:'',title:'Tambah Target Tabungan'})}>＋ Target Baru</button></div>
    {savings.length===0?<div className="card pad"><p className="empty">Belum ada target tabungan. Buat target pertama, misalnya Dana Darurat.</p></div>:<div className="cards">{savings.map(s=>{
      ...
      return <article className="card pad" key={s.id}>
        ...
        <div className="big">{rp(value)}</div>
        <p className="muted">Target {rp(target)} · Sisa {rp(Math.max(0,target-value))} · {pct.toFixed(1)}%</p>
...
function Recurring({rows,setAction,onRun,remove,refresh}:{rows:R[];setAction:(a:Action)=>void;onRun:()=>void;remove:(t:string,id:string,l:string)=>void;refresh:(m?:string)=>Promise<void>}){
  return <section>
    <div className="panel-line"><div><div className="kicker">Automation</div><h2>Transaksi Otomatis</h2></div><div className="action-row"><button className="btn alt" onClick={onRun}>Jalankan Jatuh Tempo</button><button className="btn" onClick={()=>setAction({kind:'newRecurring',id:'',title:'Tambah Jadwal Otomatis'})}>＋ Jadwal Baru</button></div></div>
    {rows.length===0?<div className="card pad"><p className="empty">Belum ada jadwal otomatis. Buat jadwal income, expense, kewajiban, atau tabungan.</p></div>:<div className="cards">{rows.map(x=><article className="card pad" key={x.id}>
      <div className="card-head"><div><span className={`badge ${x.active?'done':'pending'}`}>{x.active?'Aktif':'Pause'}</span><h3>{x.name}</h3></div><div className="action-row"><button className="icon" onClick={async()=>{await supabase.from('recurring_transactions').update({active:!x.active}).eq('id',x.id);await refresh('Jadwal diperbarui.')}}>{x.active?'Pause':'Aktifkan'}</button><button className="icon danger" onClick={()=>remove('recurring_transactions',x.id,x.name)}>Hapus</button></div></div>
      <div className="big">{rp(x.amount)}</div><p className="muted">{x.kind} · {x.frequency} · berikutnya {x.next_run}</p>
    </article>)}</div>}
  </section>;
}
...
function ActionModal({action,hid,uid,savings,wishes,parts,debts,onClose,onSaved}:{action:Exclude<Action,null>;hid:string;uid:string;savings:R[];wishes:R[];parts:R[];debts:R[];onClose:()=>void;onSaved:(m:string)=>void}){
  ...
  const newSaving=action.kind==='newSaving';
  const newRecurring=action.kind==='newRecurring';

  async function submit(e:FormEvent){
    e.preventDefault(); setBusy(true); setErr(''); let r:any;
    if(action.kind==='newSaving')r=await supabase.from('savings').insert({household_id:hid,name:name.trim(),location:location||null,target:Number(target),created_by:uid});
    else if(action.kind==='savingFund')r=await supabase.from('saving_transactions').insert({saving_id:action.id,amount:Number(amount),happened_at:date,created_by:uid});
    else if(action.kind==='debtPay')r=await supabase.from('debt_payments').insert({debt_id:action.id,amount:Number(amount),happened_at:date,created_by:uid});
    else if(action.kind==='wishlistFund')r=await supabase.from('wishlist_transactions').insert({wishlist_id:action.id,amount:Number(amount),happened_at:date,note:note||null,created_by:uid});
    else if(action.kind==='partFund')r=await supabase.from('wishlist_part_transactions').insert({part_id:action.id,amount:Number(amount),happened_at:date,note:note||null,created_by:uid});
    else if(action.kind==='editSaving')r=await supabase.from('savings').update({name:name.trim(),location:location||null,target:Number(target)}).eq('id',action.id);
    else if(action.kind==='editWishlist')r=await supabase.from('wishlists').update({name:name.trim(),brand:brand||null,target:Number(target),status}).eq('id',action.id);
    else if(action.kind==='editPart')r=await supabase.from('wishlist_parts').update({name:name.trim(),brand:brand||null,price:Number(target)}).eq('id',action.id);
    else if(action.kind==='newRecurring')r=await supabase.from('recurring_transactions').insert({household_id:hid,kind:rKind,name:name.trim(),category:category||null,amount:Number(amount),frequency,next_run:date,target_id:rKind==='saving'?savingId||null:null,active:true,created_by:uid});
    setBusy(false);
    if(r?.error)setErr(r.error.message);else onSaved(action.kind==='newSaving'?'Target tabungan berhasil dibuat.':action.kind==='newRecurring'?'Jadwal otomatis berhasil dibuat.':'Data berhasil diperbarui.');
  }

  return <Modal open title={action.title} onClose={onClose} footer={<div className="fv-modal-actions"><button className="btn alt" type="button" onClick={onClose}>Batal</button><button className="btn" form="wealth-action-v3" disabled={busy}>{busy?'Menyimpan…':'Simpan'}</button></div>}>
    <form id="wealth-action-v3" className="fv-form-grid" onSubmit={submit}>
      {newSaving&&<><label>Nama target<input value={name} onChange={e=>setName(e.target.value)} placeholder="Dana Darurat" required/></label><label>Lokasi / rekening<input value={location} onChange={e=>setLocation(e.target.value)} placeholder="Jago"/></label><label>Target dana<input type="number" min="0" value={target} onChange={e=>setTarget(e.target.value)} required/></label></>}
      {newRecurring&&<><label>Jenis<select value={rKind} onChange={e=>setRKind(e.target.value)}><option value="income">Income</option><option value="expense">Expense</option><option value="obligation">Kewajiban</option><option value="saving">Tabungan</option></select></label><label>Nama<input value={name} onChange={e=>setName(e.target.value)} required/></label><label>Nominal<input type="number" min="0" value={amount} onChange={e=>setAmount(e.target.value)} required/></label><label>Frekuensi<select value={frequency} onChange={e=>setFrequency(e.target.value)}><option value="weekly">Mingguan</option><option value="monthly">Bulanan</option><option value="yearly">Tahunan</option></select></label><label>Jadwal berikutnya<input type="date" value={date} onChange={e=>setDate(e.target.value)} required/></label>{rKind==='expense'&&<label>Kategori<input value={category} onChange={e=>setCategory(e.target.value)} placeholder="Rumah"/></label>}{rKind==='saving'&&<label>Target tabungan<select value={savingId} onChange={e=>setSavingId(e.target.value)} required><option value="">Pilih target</option>{savings.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label>}</>}
      {edit&&<><label>Nama<input value={name} onChange={e=>setName(e.target.value)} required/></label>{action.kind==='editSaving'&&<label>Lokasi / rekening<input value={location} onChange={e=>setLocation(e.target.value)}/></label>}{action.kind!=='editSaving'&&<label>Brand / model<input value={brand} onChange={e=>setBrand(e.target.value)}/></label>}<label>{action.kind==='editPart'?'Harga part':'Target'}<input type="number" value={target} onChange={e=>setTarget(e.target.value)} required/></label>{action.kind==='editWishlist'&&<label>Status<select value={status} onChange={e=>setStatus(e.target.value)}><option value="active">Berjalan</option><option value="completed">Selesai</option></select></label>}</>}
      {!newSaving&&!newRecurring&&!edit&&<><label>Nominal<input type="number" min="0" value={amount} onChange={e=>setAmount(e.target.value)} required/></label><label>Tanggal<input type="date" value={date} onChange={e=>setDate(e.target.value)} required/></label>{(action.kind==='wishlistFund'||action.kind==='partFund')&&<label className="span2">Catatan<input value={note} onChange={e=>setNote(e.target.value)} placeholder="Opsional"/></label>}</>}
