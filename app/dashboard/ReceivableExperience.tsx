...
type Mode='new'|'edit'|'payment'|null;
...
export default function ReceivableExperience({houses}:{houses:R[]}){
 ...
 async function refresh(msg?:string){await load(hid);if(msg){setTone('success');setToast(msg)}}
 async function del(r:R){if(!confirm(`Hapus piutang ${r.borrower_name} beserta seluruh riwayat cicilan?`))return;const q=await supabase.from('receivables').delete().eq('id',r.id);if(q.error){setTone('error');setToast(q.error.message)}else refresh('Piutang dihapus.')}
 async function deletePayment(p:R){if(!confirm('Hapus pembayaran cicilan ini?'))return;const q=await supabase.from('receivable_payments').delete().eq('id',p.id);if(q.error){setTone('error');setToast(q.error.message);return}await supabase.from('receivables').update({status:'belum',updated_at:new Date().toISOString()}).eq('id',p.receivable_id);refresh('Pembayaran dihapus.')}

 if(!visible||!host)return <>{mode&&<ReceivableModal mode={mode} row={selected} hid={hid} uid={uid} timezone={timezone} currentPaid={selected?paid(selected.id):0} onClose={()=>{setMode(null);setSelected(null)}} onSaved={async m=>{setMode(null);setSelected(null);await refresh(m)}}/>}<Toast message={toast} tone={tone} onDone={()=>setToast('')}/></>;
 return <>{createPortal(<section className="receivable-section">
   ...
   {sorted.length===0?<div className="receivable-empty">Belum ada piutang. Catat uang yang kamu pinjamkan agar progres pengembaliannya tidak terlupakan.</div>:<div className="receivable-grid">{sorted.map(r=>{const pRows=payments.filter(x=>x.receivable_id===r.id),p=paid(r.id),amount=Number(r.original_amount||0),left=Math.max(0,amount-p),pct=amount?Math.min(100,p/amount*100):0,done=left<=0,late=!done&&r.due_date&&r.due_date<today;return <article className={`receivable-card ${done?'is-done':''} ${late?'is-late':''}`} key={r.id}>
    ...
    {pRows.length>0&&<details className="receivable-history"><summary>Riwayat pembayaran ({pRows.length})</summary>{pRows.map(x=><div className="receivable-history-row" key={x.id}><div><b>{rp(x.amount)}</b><span>{fmt(x.paid_at)}{x.note?` · ${x.note}`:''}</span></div><button className="icon danger" onClick={()=>deletePayment(x)}>Hapus</button></div>)}</details>}
   </article>})}</div>}
   <div className="receivable-divider"><span>Hutang Saya</span><i/></div>
 </section>,host)}
 <ReceivableModal mode={mode} row={selected} hid={hid} uid={uid} timezone={timezone} currentPaid={selected?paid(selected.id):0} onClose={()=>{setMode(null);setSelected(null)}} onSaved={async m=>{setMode(null);setSelected(null);await refresh(m)}}/>
...
function ReceivableModal({mode,row,hid,uid,timezone,currentPaid,onClose,onSaved}:{mode:Mode;row:R|null;hid:string;uid:string;timezone:string;currentPaid:number;onClose:()=>void;onSaved:(m:string)=>Promise<void>}){
 ...
 useEffect(()=>{setBorrower(row?.borrower_name||'');setContact(row?.contact||'');setPurpose(row?.purpose||'');setAmount(String(row?.original_amount||''));setLentAt(row?.lent_at||today);setDue(row?.due_date||'');setRepayment(row?.repayment_type||'cicilan');setInstallments(String(row?.planned_installments||''));setInstallmentAmount(String(row?.planned_installment_amount||''));setNote(row?.note||'');setPayAmount('');setPayDate(today);setPayNote('');setErr('')},[mode,row?.id,today]);
 if(!mode)return null;
 async function submit(e:FormEvent){e.preventDefault();setErr('');setBusy(true);
  if(mode==='payment'&&row){const value=Number(payAmount||0),remaining=Math.max(0,Number(row.original_amount||0)-currentPaid);if(value<=0){setErr('Nominal pembayaran harus lebih dari 0.');setBusy(false);return}if(value>remaining){setErr(`Pembayaran melebihi sisa piutang ${rp(remaining)}.`);setBusy(false);return}const q=await supabase.from('receivable_payments').insert({receivable_id:row.id,amount:value,paid_at:payDate,note:payNote.trim()||null,created_by:uid});if(q.error){setErr(q.error.message);setBusy(false);return}const lunas=currentPaid+value>=Number(row.original_amount||0);await supabase.from('receivables').update({status:lunas?'lunas':'belum',updated_at:new Date().toISOString()}).eq('id',row.id);setBusy(false);await onSaved(lunas?'Piutang sudah lunas.':'Pembayaran piutang dicatat.');return}
  ...
  const q=mode==='edit'&&row?await supabase.from('receivables').update(payload).eq('id',row.id):await supabase.from('receivables').insert({...payload,created_by:uid});setBusy(false);if(q.error)setErr(q.error.message);else await onSaved(mode==='edit'?'Piutang diperbarui.':'Piutang baru ditambahkan.');
 }
 const title=mode==='new'?'Tambah Piutang':mode==='edit'?`Edit Piutang · ${row?.borrower_name||''}`:`Catat Pembayaran · ${row?.borrower_name||''}`;
 return <Modal open={!!mode} title={title} onClose={onClose}><form className="receivable-form" onSubmit={submit}>{mode==='payment'?<>
   <div className="receivable-payment-summary"><span>Sisa piutang</span><strong>{rp(Math.max(0,Number(row?.original_amount||0)-currentPaid))}</strong></div>
 ...
 </>}{err&&<p className="status error">{err}</p>}<div className="action-row receivable-form-actions"><button type="button" className="btn alt" onClick={onClose}>Batal</button><button className="btn" disabled={busy}>{busy?'Menyimpan…':mode==='payment'?'Simpan Pembayaran':'Simpan Piutang'}</button></div></form></Modal>;
}
