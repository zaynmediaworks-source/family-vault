...
export default function Dashboard(){
 ...
 useEffect(()=>{(async()=>{const {data,error}=await supabase.auth.getUser();if(error||!data.user){router.replace('/login');return}setUid(data.user.id);setEmail(data.user.email||'');try{const h=await loadHouse();if(h[0]){setHid(h[0].id);await loadRows(h[0].id)}}catch(e:any){setErr(e.message||'Gagal memuat data')}finally{setBusy(false)}})()},[router]);
 async function createHouse(e:FormEvent){e.preventDefault();if(!houseName.trim())return;setBusy(true);setErr('');try{const r=await supabase.from('households').insert({name:houseName.trim(),created_by:uid}).select('id,name').single();if(r.error)throw r.error;setHouses([r.data]);setHid(r.data.id);await loadRows(r.data.id)}catch(e:any){setErr(e.message||'Household gagal dibuat')}finally{setBusy(false)}}
