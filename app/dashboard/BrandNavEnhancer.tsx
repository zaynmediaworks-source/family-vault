...
export default function BrandNavEnhancer(){
 useEffect(()=>{
  ...
  const apply=()=>{
   ...
   sidebar.querySelectorAll('nav button').forEach(btn=>{const el=btn as HTMLButtonElement;if(el.dataset.brandV3)return;const label=(el.textContent||'').trim();const glyph=GLYPHS[label];if(!glyph)return;el.dataset.brandV3='1';el.textContent='';const icon=document.createElement('span');icon.className='fv-nav-glyph';icon.textContent=glyph;const text=document.createElement('span');text.textContent=label;el.append(icon,text)});
  };
